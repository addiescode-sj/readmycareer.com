...
**Result display:** Full-screen overlay modal rendered via React Portal directly into `document.body` (bypasses parent CSS `filter` constraints that would otherwise limit `position: fixed` to a transformed ancestor). Modal covers the entire viewport with a glassmorphism background (95% white opacity, 20px backdrop blur). Contains all resume sections, applied JD keyword chips, and four export actions: "Copy Markdown" (copies raw markdown to clipboard), "Download .md" (browser file download), "Copy Link" (copies `{origin}/share/{id}` to clipboard and opens the public preview in a new tab), and "PDF로 저장" (opens a print-ready HTML page and triggers the browser print/save-as-PDF dialog). PDF output uses `@page { margin: 0 }` CSS to suppress browser-generated print headers (date, title) and footers (URL, page number); all external links are normalized to `https://` before rendering. Modal closes via the × button.

**Public share page:** The URL `{origin}/share/{optimized_resume_id}` renders a publicly accessible, unauthenticated resume preview page (`/share/[id]`). The UUID primary key is cryptographically random (unguessable), so link possession acts as authorization ("anyone with the link" model). The page includes: a sticky glassmorphism header with branding and a "PDF로 저장" button; the full resume in the same section order as the modal; and a footer CTA linking back to readmycareer.com. og:title and og:description meta tags are populated for social sharing. Supabase RLS permits anonymous SELECT on `optimized_resumes` rows; authenticated access remains governed by the existing owner-only policy.
