...
> adapter makes that a one-line change, and `model_comparison.py` produces the numbers to back it.
...
- **Agent output quality** ([eval/agent_harness.py](./eval/agent_harness.py)) — schema
...
- **RAG retrieval quality** ([eval/ragas_eval.py](./eval/ragas_eval.py)) — RAGAS Faithfulness /
...
[eval/render_results.py](./eval/render_results.py):
