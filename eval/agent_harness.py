...
# place. The harness measures the Gemini product path, so it uses the "gemini" rates.
def _load_model_pricing() -> dict:
    path = Path(__file__).resolve().parent.parent / "config" / "model-pricing.json"
    with open(path, encoding="utf-8") as f:
        return json.load(f)


_PRICING = _load_model_pricing()
PRICE_INPUT_PER_1M = _PRICING["gemini"]["input"]
PRICE_OUTPUT_PER_1M = _PRICING["gemini"]["output"]
...
# Korean labels for the aggregate table, keyed by the English metric name produced by
# _summary_rows() (which the English console summary keeps using). This localizes the
# Markdown report without changing the console output.
_KO_METRIC_LABELS = {
...
# Korean glossary. Static text (definitions live in the scoring functions above); keep it
# in sync when a metric is added or its threshold rationale changes.
_METRIC_GLOSSARY = """\
| 지표 | 기준 | 의미 |
| --- | --- | --- |
...
def save_markdown_report(
    ...
    # Localize the aggregate-table metric names to Korean (console summary stays English).
    summary_rows_ko = [
        [_KO_METRIC_LABELS.get(row[0], row[0]), *row[1:]] for row in _summary_rows(metrics)
    ]
    summary_md = tabulate(
        summary_rows_ko,
        headers=["지표", "점수", "기준", "상태"],
    ...
    overall = (
        "✅ **PASS** — 모든 게이트 기준 충족"
        if all_pass
        else "❌ **FAIL** — 일부 게이트 지표가 기준 미달"
    )

    content = f"""# 에이전트 출력 품질 리포트
...
- **생성 시각:** {generated}
- **결과:** {overall}
- **케이스:** {metrics['successful_fixtures']}/{metrics['total_fixtures']} 성공
